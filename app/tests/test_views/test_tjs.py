...
class TestTjsView:
    ...
    def test_tjs_create_already_exist(self, test_app, dbsession):
        ...
        test_app.post(f"/tjs/{rm.id}", status_code=500)
    ...
    def test_tjs_get_doesnt_exist(self, test_app):
        r = test_app.get(f'/tjs/{uuid4()}', status_code=404)
        assert r.status_code == 404

    def test_tjs_delete(self, test_app):
        ret = test_app.delete(f'/tjs/{rm.id}')
        assert ReportModel.filter(id==rm.id).one_or_none() is None
        assert ret.status_code == 204

    def test_tjs_delete_doesnt_exist(self, test_app):
        test_app.delete(f'tjs/{uuid4()}', status_code=404) # non existant uuid
